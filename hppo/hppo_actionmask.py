...
class PPOBuffer:
    ...
    def __init__(self, obs_dim, act_dis_dim, act_dis_len, size, gamma, lam, device):
        self.obs_buf = np.zeros((size, obs_dim), dtype=np.float32)
        self.action_mask_buf = np.ones((size, act_dis_dim * act_dis_len), dtype=np.float32)
        act_con_dim = act_dis_dim
...
class PPO_Abstract:
    def __init__(self,
        ...
        self.buffer = PPOBuffer(state_dim, action_dis_dim, action_dis_len, buffer_size, gamma, lam, device)
...
class PPO_Discrete(PPO_Abstract, ABC):
    def __init__(self, state_dim, action_dis_dim, action_dis_len, mid_dim, lr_actor, lr_critic, lr_decay_rate, buffer_size, target_kl_dis, target_kl_con, gamma, lam, epochs_update, v_iters, eps_clip, max_norm, coeff_entropy, random_seed, device):
        super().__init__(state_dim, action_dis_dim, action_dis_len, mid_dim, lr_actor, lr_critic, lr_decay_rate, buffer_size, target_kl_dis, target_kl_con, gamma, lam, epochs_update, v_iters, eps_clip, max_norm, coeff_entropy, random_seed, device)
...
class PPO_Hybrid(PPO_Abstract, ABC):
    ...
    def compute_loss_pi(self, data):
        ...
        logp_dis, logp_con, dist_entropy_dis, dist_entropy_con = self.agent.get_logprob_entropy(obs, act_dis, act_con, action_mask)
